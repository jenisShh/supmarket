...
const Sandwiches = () => {
	...
	const Sandwich = ({sandwich}) => {
		return(
            ...
            <Card.Img  src={`${sandwich.images[0].imageName}`} />
            <Card.Body>
                <Card.Title>
                    {`${sandwich.title}`}
                </Card.Title>
                <Card.Subtitle className= "mb-2 text-muted">  
                    {`${sandwich.categoryTitle}`}
                </Card.Subtitle>
				<p>NRS {`${sandwich.unitPrice[0].sellingPrice}`}</p> 
    ...
    return (
      ...
      <p className="mt-2" >We've got something for everyone</p>
       <Row>
                {loading && 
                    sandwiches.filter((sandwich) => sandwich.categoryId===6).map((sandwich) => (
                        <Col sm={12} md={6} lg={4} key= {sandwich.id}> 
