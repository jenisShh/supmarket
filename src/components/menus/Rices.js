...
const Rices = () => {
	...
	const Rice = ({rice}) => {
		return(
			...
			<Card className = "ml-3 my-3 p-3 rounded h-90">
           		 <Card.Img  src={`${rice.images[0].imageName}`} />
					<Card.Body>
						<Card.Title>
							{`${rice.title}`}
						</Card.Title>
						...
						<p>NRS {`${rice.unitPrice[0].sellingPrice}`}</p> 
    ...
    return (
      ...
      <p className="mt-2" >We've got something for everyone</p>
       <Row>
                {loading && 
                    rices.filter((rice) => rice.categoryId===8).map((rice) => (
                        <Col sm={12} md={6} lg={4} key= {rice.id}> 
                        <Rice rice = {rice} />
                        </Col>
                )) }
