...
const Healthychoices = () => {
	...
	const getHealthychoices = async () =>{
		try {
			const res = await axios.get(
			  ...
			  setHealthychoices(res.data.data);
			  setLoading(true);
	...
	const Healthychoice = ({healthychoice}) => {
		return(
			<>
			
			<Card className = "ml-3 my-3 p-3 rounded h-90">
            <Card.Img  src={`${healthychoice.images[0].imageName}`} />
            <Card.Body>
                <Card.Title>
                    {`${healthychoice.title}`}
                </Card.Title>
                <Card.Subtitle className= "mb-2 text-muted">  
                    {`${healthychoice.categoryTitle}`}
                </Card.Subtitle>
				<p>NRS {`${healthychoice.unitPrice[0].sellingPrice}`}</p> 
    ...
    return (
            ...
            <div class="breadcrumbs">
                <div class="container">
                    <ol class="breadcrumb breadcrumb1 animated wow slideInLeft" data-wow-delay=".5s">
                        ...
                        <li class="active">HEALTHY CHOICE</li>
                    </ol>
                </div>
	        </div>
      ...
      <p className="mt-2" >We've got something for everyone</p>
       <Row>
                {loading && 
                    healthychoices.filter((healthychoice) => healthychoice.categoryId===7).map((healthychoice) => (
                        <Col sm={12} md={6} lg={4} key= {healthychoice.id}> 
                        <Healthychoice healthychoice = {healthychoice} />
                        </Col>
                )) }
